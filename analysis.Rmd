...
We initially consulted the  “LexiCoder Policy Agenda” dictionary to have a baseline for words that could capture these main topics. We then translated to Portuguese and adapted to Brazilian context. 
...
wf_econ <- textmodel_wordfish(dfmat_econ, dir = c(19, 11), sparse = TRUE) # PT and PR
...
In general, the right discusses angrucultura (agribusiness), with words like empreendedorismo ("entrepeneurship")/ liberalismo ("liberalism"), logística ("logistics") /infraestrutura ("infrastructure").
...
Interestingly, the use of cadeia ("jail") and prisão ("prison"), which are synonyms in Portuguese, seem associated with speeches in different sides of the RILE spectrum. Similarly, the word vagabundagem (iddleness that characterizes a tramp) and vagabundo (s) ("tramp") are also associated with different sides of RILE.

Finally, "feminicídio" (a relatively new criminal offense for an aggravated homicide committed against a woman by her former romantic partner), which is a diferenciation to standard homicide typically criticized by right wing politicians with a conservative view of gender issues, is the word that appears most strongly associated with a right-leaning political view.
...
wf_genfam <- textmodel_wordfish(dfmat_genfam, dir = c(19, 11), sparse = TRUE) # PT and PR
...
```{r}
words_genfam <- c('kit gay', 'just*', 'tradicional', 'mulher', 'gênero', 'feminis*', 'juventude', 'gay',
                  'jovem', 'família')

scores_genfam <- arrange(filter(feat_genfam, word %in% words_genfam), beta)

##### mostrar palavras mais à direita e à esquerda
scores_genfam %>% 
  arrange(desc(beta)) %>%  # tirar o desc para mostrar palavras da esquerda == arrange(beta)
  select(word)
