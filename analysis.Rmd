...
### Topic Model Presidential Manifestos

```{r Topic_Model_Manifestos}
lda_pres <- LDA(dfmat_pres, k = 6, control = list(seed = 123))

topics_pres <- tidy(lda_pres, matrix = "beta")
...
```{r}
# gamma --  probability of the words in document X that were generated from topic X
gamma_pres <- tidy(lda_pres, matrix = "gamma")

# table higher gamma per project
gamma_pres %>% 
  group_by(document) %>% 
  slice_max(gamma, n = 3, with_ties = TRUE) %>% 
  filter(gamma > 0.8) %>% 
  arrange(topic) %>% 
  rename(Party = 'document', Topic = 'topic', Gamma = 'gamma')
```
...
```{r}
words_econ <- c('trabalhadores', 'direitos', 'aposentados', 'pensionistas', 'escrvavidão',
...
# feat_econ %>% 
#   filter(beta > 0.5 & beta < 1 & psi < -2.6 & psi > -6)
...
```{r}
words_genfam <- c('kit gay', 'just*', 'tradicional', 'mulher', 'gênero', 'feminis*', 'juventude', 'gay', 'jovem', 'família')

scores_genfam <- arrange(filter(feat_genfam, word %in% words_genfam),
                      beta)
