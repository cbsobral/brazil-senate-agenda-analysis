...
Let's start by briefly describing the data we will use in our analysis. If you are are curious to know the preliminary steps we took to assemble this information, please find our initial codes in the [Github](https://github.com/cbsobral/tada/tree/main/initial) page for this project. The two data frames containing Senate information were downloaded through [Brazilian Senate API](https://www12.senado.leg.br/dados-abertos).

- `sen_df: a data frame containing information on Senators (name, gender, term in office, party affiliation,State, etc.).
- `speech_df: a data frame containing all speeches from Brazilian Senators for the period of 2019-2019.
- `cemf_2019: a data frame containing a Brazilian press assessment of parties' positions
- `pres_df: a data frame containing all manifestos from Brazilian Presidentian Election of 2018.
...
```
In the graph below, the parties represented in blue have spoken more than their expected average, considering the number of Senators they hold. Parties represented in red have spoken less than their expected average. This finding suggests that parties from the left, noticeably, PT and REDE, speak more than their pairs.

This might lead to a higher opposition to Bolsonaro's agenda within the Senate corpus.
...
One final thing needs to be mentioned about Brazilian Politics current state of affairs. In recent years, politicians in Brazil have often changed parties. This happens mostly out of ideological differences and conflicts with Party leadership. In fact, Bolsonaro has been party-less ("S/ Partido") for over a year.

The lack of party loyalty and identification that is currently a trend in Brazilian Political context might render any study that attempts to draw an inference about party position from politicians' speeches susceptible to criticism. Katia Abreu, for instance, has been associated with 3 different Parties in the period of 2019-2020, and has also been party-less for a while. In order to assess the degree to which this study is vulnerable to this criticism, we listed bellow the changes in party affiliation that affect our corpus.
...
The plot shows PL as the party farthest to the right and PT as the one farthest to the left.
