...
date: "`r Sys.Date()`" 
abstract: |
  In this study, we examine the collection of speeches made in the Brazilian Senate in the period of 2019-2020 to assess how three dimensions of Bolsonaro’s political agenda resonate within the discussions being held in the Senate House. We start by exploring the composition of the Brazilian Senate, and we find that speech distribution is not evenly balanced, but disproportionally represent male speeches, and coming from a few states and with a predominance of parties from the left. 
  Turning to scaling, we compare party ideologies in the Right-Left scale based on Senate’s speeches with an assessment made by the Brazilian press. Then we compare party positions using the manifestos from the 2018 presidential elections.
  Finally, in assessing the three dimensions of Bolsonaro’s agenda, we conclude that economic discussions seem more polarized, with a distinct preference for certain words and topics, depending on RILE position. Discussions based on security and gender & family are less prevalent in the corpus, and do not suggest a strong differentiation in the vocabulary employed.
output: 
  html_document:
    code_folding: hide 
    toc: yes
