...
sum(sen_df$party_abbr== "PT")/80

sum(speech_df$Partido =="PT")/5188
```
...
Our dataset is composed of 5188 speeches given by 80 Senators of 27 States.
Assuming each Senator uses speech opportunities equally, we would expect each State to have around 192 speeches. Let's see if this checks out.
...
lda_sen <- LDA(dfmat_sen, k = 6, control = list(seed = 123))

topics_sen <- tidy(lda_sen, matrix = "beta")

top_terms_sen <- topics_sen %>%
  group_by(topic) %>%
  top_n(20, beta) %>%
  ungroup() %>%
  arrange(topic, -beta) %>% 
...
#gamma --  XX% of the words in document X that were generated from topic X
gamma_sen <- tidy(lda_sen, matrix = "gamma")
