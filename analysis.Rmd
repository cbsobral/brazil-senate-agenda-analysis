...
Economy, Security and Gender & Family. 
...
To extract documents that discuss economic topics, we filter documents that have higher scores on the economy dictionary when compared to the security and gender & family dictionaries. 
...
Comparing the economy graph for Wordfish with the main graph, we do not see much difference in party positions.
...
The words that most contributed for a left-leaning document are *petrobrás, anticapitalista, cut,  auxílio-doença, sindicalista* and *terceirização*. For the right, the words are *militarismo, agroindustriais, grevista, veterinária, logística* and *transportes*. 

In general, the right discusses = agricultura, with words like (...), empreendedorismo/liberalismo, logística/infraestrutura.

The left = previdência, with words like (...), trabalhadores, direitos, macroeconomia, sindicalismo.
...
To extract documents that discuss security topics, we filter documents that have higher scores on the security dictionary when compared to the economy and gender & family dictionaries.
...
### Scaling
Comparing the security graph for Wordfish with the main graph, we see a lot of change in party positions.
...
Finally, "feminicídio" (a releatively new tipification for an aggravated homicide committed against a woman by her former romantic partner), which is a crime typically criticized by right wing politicians with a conservative view of gender issues, is the word that appears most strongly associated with a right-leaning political view.
