...
Acho que vale explicar o que tem nas duas. # carol
...
# arrumar linha -- em 45º mesmo numero nas duas dfs
t_comp %>% 
  arrange(theta_pres) %>%    
  ...
  geom_point() + 
  geom_label_repel() +
  geom_abline(intercept = 1.2, slope = 1) +
  scale_y_continuous(breaks = c(-1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5)) +
  labs(x = "Theta Senate", y = "Theta President") +
  theme_classic()
  
  

# gráfico com duas linhas por partido no eixo y
t_comp %>%
  arrange(theta_pres) %>%    
  mutate(party = factor(party, levels = party)) %>% 
  ggplot(aes(x = theta_pres, y = party)) + 
  geom_point() + 
  theme_minimal()
```
