...
```{r}
# Load data frame
load("/cloud/project/senate/data/sen_df.Rda")
glimpse(sen_df)
```

```{r corpus}
# Create corpus
sen_corpus <- corpus(sen_df, docid_field = "CodigoPronunciamento", 
                     text_field = "Pronunciamento")
```


```{r dfm eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# Create DFM
## Não roda no Cloud. Tirei do knit e coloquei load() ali embaixo. ##
...
```{r dfm clean}
# Load DFM
load("/cloud/project/senate/data/sen_dfm.Rda") # completa, sem filtro nenhum
...
```{r}
# Keyword in context
head(kwic(sen_corpus, pattern = "deus", window = 3, valuetype = "glob"))
```
