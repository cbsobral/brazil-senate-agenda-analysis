...
```{r}
# dictionary
# economy from 'dictionaries/policy_agendas_english.RData'
...
dfmat_theme <- dfm(corp_sen, dictionary = theme.lexicon)
dfmat_theme <- dfm_weight(dfmat_theme, scheme = 'prop')
convert(dfmat_theme, 'data.frame')

dfm_lookup(dfmat_sen, theme.lexicon, valuetype = "glob")

econ_docid <- convert(dfmat_theme, 'data.frame') %>% 
  filter(economy > social) %>% 
  select(doc_id) %>% 
  as.list(doc_id)

summary(corpus_subset(corp_sen, id %in% econ_docid))
